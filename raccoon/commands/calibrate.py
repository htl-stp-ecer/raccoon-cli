...
def _calibrate_local(ctx: click.Context, project_root: Path, config: dict, aggressive: bool) -> None:
    """Run calibration locally (on the Pi itself)."""
...
async def _calibrate_remote(ctx: click.Context, project_root: Path, config: dict, aggressive: bool) -> None:
    """Run calibration on the connected Pi."""
    console: Console = ctx.obj["console"]
...
@click.pass_context
def calibrate_command(ctx: click.Context, aggressive: bool, local: bool) -> None:
    ...
    if not local:
        ...
        if manager.is_connected:
            # Run remotely
            asyncio.run(_calibrate_remote(ctx, project_root, config, aggressive))
            return

    # Run locally
    _calibrate_local(ctx, project_root, config, aggressive)
