...
from raccoon_cli.server.routes.commands import _active_commands
...
def setup_websocket_routes(app: FastAPI) -> None:
    ...
    async def websocket_output(
        ...
        - Server sends JSON tagged with {"__raccoon": "control", ...} for
          protocol messages (final status, errors, cancel ack). Untagged
          JSON is treated as program output, so lines that happen to look
          like JSON don't collide with the control channel.
        - Final status: {"__raccoon": "control", "kind": "status",
          "status": "completed", "exit_code": N, "finished_at": ...}
    ...
    try:
        while True:
            ...
            if data.get("action") == "cancel":
                await executor.cancel()
                await websocket.send_json(
                    {"__raccoon": "control", "kind": "cancelling"}
                )
